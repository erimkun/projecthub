...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (fields.length > 0) {
    ...
    await db.prepare(`UPDATE notes SET ${fields.join(', ')} WHERE id = ?`).run(...values);
  }

  // Link a task to this note
  if (body.link_task_id) {
    await db.prepare('INSERT OR IGNORE INTO note_tasks (note_id, task_id) VALUES (?, ?)')
      .run(Number(id), body.link_task_id);
  }

  const note = await db.prepare('SELECT * FROM notes WHERE id = ?').get(Number(id));
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const db = await getDb();
  const { id } = await params;
  await db.prepare('DELETE FROM notes WHERE id = ?').run(Number(id));
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const note = await db.prepare('SELECT * FROM notes WHERE id = ?').get(Number(id)) as { project_id?: number } | undefined;
